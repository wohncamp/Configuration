...
if [ -d "$PLUGIN_DIR/vim-lsp" ]; then
  ...
  git clone https://github.com/prabirshrestha/vim-lsp.git $PLUGIN_DIR/vim-lsp
fi

if [ -d "$PLUGIN_DIR/vim-lsp-settings" ]; then
  git -C "$PLUGIN_DIR/vim-lsp-settings" pull
else
  git clone https://github.com/mattn/vim-lsp-settings.git $PLUGIN_DIR/vim-lsp-settings
fi

if [ -d "$PLUGIN_DIR/asyncomplete.vim" ]; then
  git -C "$PLUGIN_DIR/asyncomplete.vim" pull
else
  git clone https://github.com/prabirshrestha/asyncomplete.vim.git $PLUGIN_DIR/asyncomplete.vim
fi

if [ -d "$PLUGIN_DIR/asyncomplete-lsp.vim" ]; then
  git -C "$PLUGIN_DIR/asyncomplete-lsp.vim" pull
else
  git clone https://github.com/prabirshrestha/asyncomplete-lsp.vim.git $PLUGIN_DIR/asyncomplete-lsp.vim
fi
